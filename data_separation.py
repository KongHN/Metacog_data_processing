...
def split_data(data):
    """
    根据 'Block' 列将数据框均分为两部分
    :param data: 要分割的 pandas 数据框，需含 'Block' 列
    ...
    first_half = data[data['Block'].isin(unique_blocks[:half_index])]
    second_half = data[data['Block'].isin(unique_blocks[half_index:])]
...
def process_file(input_file_path, output_folder):
    ...
    :return: 文件处理成功返回 True，否则返回 False
    """
